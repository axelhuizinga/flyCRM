...
class model_QC extends model_Clients {
	...
	public function errSale($q) {
		...
		$user = S::$user;
		$sql = "UPDATE vicidial_list SET status='NI', list_id=`entry_list_id` WHERE lead_id=" . _hx_string_rec($lead_id, "");
		$this->query($sql);
	...
	public function save($q) {
		...
		if($log_id) {
			...
			call_user_func_array($tmp1, array($tmp2, _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 174, "className" => "model.QC", "methodName" => "save"))));
			$primary_id = S::$my;
			$primary_id1 = $primary_id->real_escape_string($q->get("primary_id"));
			$sql = new StringBuf();
			...
			call_user_func_array($tmp3, array($tmp4, _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 179, "className" => "model.QC", "methodName" => "save"))));
			...
			$sets = new _hx_array(array());
			{
				...
				while($_g < $cFields->length) {
					...
					if($val !== null) {
						...
						unset($type,$tmp6,$tmp5,$bindTypes3,$bindTypes2,$bindTypes1);
					}
					unset($val,$c);
				}
			}
			...
			$stmt = S::$my->stmt_init();
			haxe_Log::trace($sql->b, _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 202, "className" => "model.QC", "methodName" => "save")));
			haxe_Log::trace($values2bind, _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 203, "className" => "model.QC", "methodName" => "save")));
			...
			if($tmp7) {
				haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 207, "className" => "model.QC", "methodName" => "save")));
				return false;
			}
			if($i > 0) {
			...
			$tmp8 = (property_exists("haxe_Log", "trace") ? haxe_Log::$trace: array("haxe_Log", "trace"));
			$tmp9 = "success:" . Std::string($success);
			call_user_func_array($tmp8, array($tmp9, _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 212, "className" => "model.QC", "methodName" => "save"))));
			if($success) {
				if($i !== 0) {
				...
				if(!$success) {
					haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 218, "className" => "model.QC", "methodName" => "save")));
					return false;
				}
				if($i === 0) {
					haxe_Log::trace("i:" . _hx_string_rec($i, "") . " custom fields saved???", _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 222, "className" => "model.QC", "methodName" => "save")));
				}
				$sql = new StringBuf();
				...
				$sets = new _hx_array(array());
				{
					...
					while($_g1 < $uFields->length) {
						...
						if($val1 !== null) {
							...
							unset($type1,$tmp10,$bindTypes6,$bindTypes5,$bindTypes4);
						}
						unset($val1,$c1);
					}
				}
				...
				if(!_hx_equal($q->get("status"), "QCOK")) {
					$tmp11 = _hx_equal($q->get("status"), "QCBAD");
				} else {
					$tmp11 = true;
				}
				if($tmp11) {
					...
					haxe_Log::trace("list_id:" . _hx_string_rec($list_id, ""), _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 265, "className" => "model.QC", "methodName" => "save")));
					$entry_list_id = $q->get("entry_list_id");
					$i = $i + 1;
				...
				haxe_Log::trace($sql->b, _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 283, "className" => "model.QC", "methodName" => "save")));
				$success1 = $stmt1->prepare($sql->b);
				if(!$success1) {
					haxe_Log::trace($stmt1->error, _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 287, "className" => "model.QC", "methodName" => "save")));
					return false;
				}
				haxe_Log::trace("" . _hx_string_rec($i, "") . " values:", _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 290, "className" => "model.QC", "methodName" => "save")));
				haxe_Log::trace($values2bind, _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 291, "className" => "model.QC", "methodName" => "save")));
				$success1 = myBindParam($stmt1, $values2bind, $bindTypes);
				$tmp12 = (property_exists("haxe_Log", "trace") ? haxe_Log::$trace: array("haxe_Log", "trace"));
				$tmp13 = "success:" . Std::string($success1);
				call_user_func_array($tmp12, array($tmp13, _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 293, "className" => "model.QC", "methodName" => "save"))));
				if($success1) {
					$success1 = $stmt1->execute();
					if(!$success1) {
						haxe_Log::trace($stmt1->error, _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 299, "className" => "model.QC", "methodName" => "save")));
						return false;
					} else {
			...
			haxe_Log::trace("oops", _hx_anonymous(array("fileName" => "QC.hx", "lineNumber" => 312, "className" => "model.QC", "methodName" => "save")));
		}
		return false;
	}
