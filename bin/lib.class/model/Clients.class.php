...
class model_Clients extends Model {
	...
	public function edit($param) {
		$entry_list_id = S::$my;
		$entry_list_id1 = $entry_list_id->real_escape_string($param->get("entry_list_id"));
		...
		$_g1->set("anrede", "Frau,Frau\x0D\x0AHerr,Herr\x0D\x0AFamilie,Familie\x0D\x0AFirma,Firma");
		$optionsMap = $_g1;
		...
		{
			$_g21 = 0;
			while($_g21 < $tableNames->length) {
				$table = $tableNames[$_g21];
				$_g21 = $_g21 + 1;
				...
				case "buchungs_anforderungen":{
					...
					$p->set("where", "`Mandat-ID`|LIKE|" . _hx_string_rec(Std::parseInt($param->get("client_id")), "") . "_1");
					$p->set("limit", "2400");
				...
				case "clients":{
					...
					$p->set("where", "vicidial_list.lead_id|" . _hx_string_rec(Std::parseInt($param->get("lead_id")), ""));
					$tmp1 = $this->doJoin($p, $sb, $phValues);
					$editTables->set($table, php_Lib::hashOfAssociativeArray($tmp1));
					haxe_Log::trace($editTables, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 181, "className" => "model.Clients", "methodName" => "edit")));
				}break;
				default:{
					$tmp2 = null;
					...
					if($table === "vicidial_list") {
						$p->set("where", "vendor_lead_code|" . _hx_string_rec(Std::parseInt($param->get("client_id")), ""));
					} else {
						$p->set("where", "client_id|" . _hx_string_rec(Std::parseInt($param->get("client_id")), ""));
					}
					...
					if($table === "pay_source") {
						...
						call_user_func_array($tmp4, array($tmp5, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 202, "className" => "model.Clients", "methodName" => "edit"))));
					}
				}break;
				}
		...
		$_g22 = new haxe_ds_StringMap();
		$_g22->set("where", "reason|AC01 AC04 AC06 MD06 MS03,m_ID|" . _hx_string_rec(Std::parseInt($param->get("client_id")), ""));
		$_g22->set("limit", 150);
		$editTables->set("konto_auszug", php_Lib::hashOfAssociativeArray(_hx_cast($tmp6->findClient($_g22, true), _hx_qtype("php.NativeArray"))));
	...
	public function saveLog($q, $ref_id = null) {
		...
		if(S::$my->query("INSERT INTO vicidial_lead_log SELECT * FROM (SELECT NULL AS log_id," . _hx_string_rec($lead_id, "") . " AS lead_id,NOW() AS entry_date) AS ll JOIN (SELECT modify_date,status,user,vendor_lead_code,source_id,list_id,gmt_offset_now,called_since_last_reset,phone_code,phone_number,title,first_name,middle_initial,last_name,address1,address2,address3,city,state,province,postal_code,country_code,gender,date_of_birth,alt_phone,email,security_phrase,comments,called_count,last_local_call_time,rank,owner,entry_list_id," . _hx_string_or_null($user) . " AS log_user," . _hx_string_rec($ref_id, "") . " as ref_id FROM `vicidial_list` WHERE `lead_id`=" . _hx_string_rec($lead_id, "") . ")AS vl", null)) {
			...
			if($this->checkOrCreateCustomTable($cTable, null)) {
				...
				haxe_Log::trace("INSERT INTO " . _hx_string_or_null($cLogTable) . " ..." . _hx_string_or_null(S::$my->error) . "<", _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 284, "className" => "model.Clients", "methodName" => "saveLog")));
				if(S::$my->error === "") {
					return $log_id;
				}
			}
	...
	public function savePayBack($q) {
		haxe_Log::trace($q, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 294, "className" => "model.Clients", "methodName" => "savePayBack")));
		$client_id = S::$my;
		$client_id1 = $client_id->real_escape_string($q->get("client_id"));
		$amount = Std::parseInt($q->get("Betrag"));
		$buchungs_datum = S::$my;
	...
	public function save($q) {
		...
		if($ref_id) {
			...
			call_user_func_array($tmp, array($tmp1, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 340, "className" => "model.Clients", "methodName" => "save"))));
			...
			$sets = new _hx_array(array());
			{
				...
				while($_g < $cFields->length) {
					...
					call_user_func_array($tmp2, array($tmp7, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 351, "className" => "model.Clients", "methodName" => "save"))));
					if($val !== null) {
						$i = $i + 1;
						$tmp8 = null;
			...
			if(!$success) {
				haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 376, "className" => "model.Clients", "methodName" => "save")));
				return false;
			}
			...
			if($success) {
				if($customFields2Save) {
					...
					if(!$success) {
						haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 393, "className" => "model.Clients", "methodName" => "save")));
						return false;
					}
				}
				$sql = new StringBuf();
				...
				haxe_Log::trace($sql->b, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 455, "className" => "model.Clients", "methodName" => "save")));
				$success1 = $stmt1->prepare($sql->b);
				if(!$success1) {
					haxe_Log::trace($stmt1->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 459, "className" => "model.Clients", "methodName" => "save")));
					return false;
				}
				...
				call_user_func_array($tmp14, array($tmp15, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 465, "className" => "model.Clients", "methodName" => "save"))));
				if($success1) {
					$success1 = $stmt1->execute();
					if(!$success1) {
						haxe_Log::trace($stmt1->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 471, "className" => "model.Clients", "methodName" => "save")));
						return false;
					}
					if(!$this->saveClientData($q)) {
						haxe_Log::trace("oops:" . _hx_string_or_null(S::$my->error), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 476, "className" => "model.Clients", "methodName" => "save")));
					} else {
						return !_hx_equal($this->saveLog($q, $ref_id), false);
					}
				} else {
					haxe_Log::trace("oops:" . _hx_string_or_null(S::$my->error), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 484, "className" => "model.Clients", "methodName" => "save")));
				}
			}
		}
		return false;
	...
	public function saveClientDataLog($q, $ref_id = null) {
		...
		if(!$tmp) {
			...
			call_user_func_array($tmp2, array($tmp3, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 498, "className" => "model.Clients", "methodName" => "saveClientDataLog"))));
			return false;
		}
	...
	public function saveClientData($q) {
		...
		$stmt = S::$my->stmt_init();
		haxe_Log::trace($sql->b, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 540, "className" => "model.Clients", "methodName" => "saveClientData")));
		$success = $stmt->prepare($sql->b);
		if(!$success) {
			haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 544, "className" => "model.Clients", "methodName" => "saveClientData")));
			return false;
		}
		haxe_Log::trace($values2bind, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 548, "className" => "model.Clients", "methodName" => "saveClientData")));
		$success = myBindParam($stmt, $values2bind, $bindTypes);
		$tmp2 = (property_exists("haxe_Log", "trace") ? haxe_Log::$trace: array("haxe_Log", "trace"));
		$tmp3 = "success:" . Std::string($success);
		call_user_func_array($tmp2, array($tmp3, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 550, "className" => "model.Clients", "methodName" => "saveClientData"))));
		if($success) {
			$success = $stmt->execute();
			if(!$success) {
				haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 556, "className" => "model.Clients", "methodName" => "saveClientData")));
				return false;
			}
	...
	public function save_pay_plan($q) {
		...
		call_user_func_array($tmp, array($tmp3, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 582, "className" => "model.Clients", "methodName" => "save_pay_plan"))));
		$pIt = $product->keys();
		while($pIt->hasNext()) {
			$pay_plan_id = $pIt->next();
			$log_id = $this->save_pay_plan_log($pay_plan_id, null);
			haxe_Log::trace($log_id, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 590, "className" => "model.Clients", "methodName" => "save_pay_plan")));
			if(!$log_id) {
				return false;
			}
			$sql = new StringBuf();
			...
			$sql->add("UPDATE fly_crm.pay_plan SET ");
			{
				...
				while($_g < $uFields->length) {
					...
					call_user_func_array($tmp4, array($tmp5, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 605, "className" => "model.Clients", "methodName" => "save_pay_plan"))));
					$p = $q->get($c);
					$val = null;
					if($p !== null) {
			...
			call_user_func_array($tmp6, array($tmp7, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 643, "className" => "model.Clients", "methodName" => "save_pay_plan"))));
			if($success) {
				$success = $stmt->execute();
				if(!$success) {
	...
	public function save_pay_source($q) {
		...
		call_user_func_array($tmp, array($tmp2, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 676, "className" => "model.Clients", "methodName" => "save_pay_source"))));
		$pIt = $account->keys();
		$user = S::$user;
		while($pIt->hasNext()) {
			...
			$sql->add("UPDATE fly_crm.pay_source SET ");
			{
				...
				while($_g < $uFields->length) {
					...
					call_user_func_array($tmp3, array($tmp4, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 699, "className" => "model.Clients", "methodName" => "save_pay_source"))));
					$p = $q->get($c);
					$val = null;
					if($p !== null) {
						if(!Std::is($p, _hx_qtype("String"))) {
			...
			call_user_func_array($tmp5, array($tmp6, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 737, "className" => "model.Clients", "methodName" => "save_pay_source"))));
			if($success) {
				$success = $stmt->execute();
				if(!$success) {
...
model_Clients::$clients_fields = _hx_explode(",", "client_id,lead_id,creation_date,state,use_email,register_on,register_off,register_off_to,teilnahme_beginn,title,anrede,namenszusatz,co_field,storno_grund,birth_date,old_active");
model_Clients::$pay_history_fields = _hx_explode(",", "buchungsanforderungID,Mandat-ID,Betrag,Termin,tracking_status,Textschlüssel bzw. Zahlart");
