...
class model_ClientHistory extends model_Clients {
	...
	public function findClient($param, $dataOnly = null) {
		...
		if(_hx_index_of($reasons, "MD06", null) > -1) {
			...
			$sql->add("(SELECT d, e AS amount, SUBSTR(j,17,8) AS m_ID, z AS IBAN, \"MD06\" AS reason FROM `konto_auszug` \x0D\x0A\x09\x09\x09WHERE (i LIKE \"%MD06%\" OR i LIKE \"%AC06%\"\x0D\x0A\x09\x09\x09" . _hx_string_or_null($cond) . "  LIMIT 0,10000)");
			$sql->add("UNION ");
			$sql->add("(SELECT d, e AS amount, SUBSTR(k, 17, 8) AS m_ID, z AS IBAN, \"MD06\" AS reason FROM `konto_auszug` WHERE j LIKE \"%MD06%\" " . _hx_string_or_null($cond) . "  LIMIT 0, 10000)");
		}
	...
	public function find($param) {
		$sb = new StringBuf();
		$phValues = new _hx_array(array());
		haxe_Log::trace($param, _hx_anonymous(array("fileName" => "ClientHistory.hx", "lineNumber" => 230, "className" => "model.ClientHistory", "methodName" => "find")));
		...
		call_user_func_array($tmp, array(_hx_string_or_null($tmp2) . _hx_string_or_null($tmp3), _hx_anonymous(array("fileName" => "ClientHistory.hx", "lineNumber" => 235, "className" => "model.ClientHistory", "methodName" => "find"))));
		$tmp4 = null;
		if($param->exists("page")) {
