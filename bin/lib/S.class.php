...
class S {
	...
	static function main() {
		...
		php_Session::start();
		haxe_Log::trace(dumpSession(), _hx_anonymous(array("fileName" => "S.hx", "lineNumber" => 40, "className" => "S", "methodName" => "main")));
		...
		haxe_Log::trace($params, _hx_anonymous(array("fileName" => "S.hx", "lineNumber" => 55, "className" => "S", "methodName" => "main")));
		...
		$auth = S::checkAuth();
		haxe_Log::trace(_hx_string_or_null($action) . ":" . Std::string($auth), _hx_anonymous(array("fileName" => "S.hx", "lineNumber" => 69, "className" => "S", "methodName" => "main")));
		if(!$auth) {
			S::hexit("AUTH FAILURE");
			return;
		}
		$result = Model::dispatch($params);
		haxe_Log::trace($result, _hx_anonymous(array("fileName" => "S.hx", "lineNumber" => 77, "className" => "S", "methodName" => "main")));
		if(!S::$headerSent) {
	...
	static function checkAuth() {
		$user = php_Session::get("PHP_AUTH_USER");
		if($user === null) {
			return false;
		}
		haxe_Log::trace($user, _hx_anonymous(array("fileName" => "S.hx", "lineNumber" => 92, "className" => "S", "methodName" => "checkAuth")));
		$pass = php_Session::get("PHP_AUTH_PW");
		if($pass === null) {
			return false;
		}
		haxe_Log::trace($pass, _hx_anonymous(array("fileName" => "S.hx", "lineNumber" => 96, "className" => "S", "methodName" => "checkAuth")));
		...
		if(S_0($pass, $res, $user) === "1") {
			S::hexit("ENCRYPTED PASSWORDS NOT IMPLEMENTED");
		}
		$res = php_Lib::hashOfAssociativeArray(_hx_deref(new Model())->query("SELECT count(*) AS cnt FROM vicidial_users WHERE user=\"" . _hx_string_or_null($user) . "\" and pass=\"" . _hx_string_or_null($pass) . "\" and user_level > 7 and active=\"Y\""));
		return $res->exists("0") && S_1($pass, $res, $user) === "1";
	}
