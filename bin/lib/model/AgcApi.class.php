...
=======
				$this->data = _hx_anonymous(array("response" => "OK", "choice" => $this->query("(SELECT `status`,`status_name` FROM `vicidial_statuses` WHERE `selectable`=\"Y\") UNION (SELECT `status`,`status_name` FROM `vicidial_campaign_statuses` WHERE `selectable`=\"Y\" AND campaign_id=\"" . _hx_string_or_null($campaign_id->real_escape_string($param->get("campaign_id"))) . "\") ORDER BY `status`")));
>>>>>>> a32fff50bcc6512f1e0fbc14f62d7da0632f1c7d
...
=======
		$tmp = null;
		if($agcResponse->query("UPDATE vicidial_list SET state=\"" . _hx_string_or_null($state) . "\" WHERE lead_id=" . Std::string($lead_id), null)) {
>>>>>>> a32fff50bcc6512f1e0fbc14f62d7da0632f1c7d
			$tmp = "OK";
		} else {
			$tmp = S::$my->error;
