...
class model_AgcApi extends Model {
	...
	public function check4Update($param) {
		...
		return $this->json_response(_hx_array_get($this->query("SELECT security_phrase FROM vicidial_list WHERE lead_id=" . Std::string($lead_id)), 0));
	}
	...
	public function update_fields_x($param) {
		$security_phrase = $param->get("security_phrase");
		$lead_id = Std::parseInt($param->get("lead_id"));
		$agcResponse = S::$my->query("UPDATE vicidial_list SET security_phrase=\"" . _hx_string_or_null($security_phrase) . "\" WHERE lead_id=" . Std::string($lead_id), null);
		return $this->json_response(model_AgcApi_0($this, $agcResponse, $lead_id, $param, $security_phrase));
	}
...
function model_AgcApi_0(&$__hx__this, &$agcResponse, &$lead_id, &$param, &$security_phrase) {
	if($agcResponse) {
		return "OK";
	} else {
		return S::$my->error;
