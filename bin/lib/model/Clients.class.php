...
class model_Clients extends Model {
	...
	public function saveLog($q, $ref_id = null) {
		...
		if(S::$my->query("INSERT INTO vicidial_lead_log SELECT * FROM (SELECT NULL AS log_id," . _hx_string_rec($lead_id, "") . " AS lead_id,NOW() AS entry_date) AS ll JOIN (SELECT modify_date,status,user,vendor_lead_code,source_id,list_id,gmt_offset_now,called_since_last_reset,phone_code,phone_number,title,first_name,middle_initial,last_name,address1,address2,address3,city,state,province,postal_code,country_code,gender,date_of_birth,alt_phone,email,security_phrase,comments,called_count,last_local_call_time,rank,owner,entry_list_id," . _hx_string_or_null($user) . " AS log_user," . _hx_string_rec($ref_id, "") . " as ref_id FROM `vicidial_list` WHERE `lead_id`=" . _hx_string_rec($lead_id, "") . ")AS vl", null)) {
			...
			if($this->checkOrCreateCustomTable($cTable, null)) {
				...
				haxe_Log::trace("INSERT INTO " . _hx_string_or_null($cLogTable) . " ..." . _hx_string_or_null(S::$my->error) . "<", _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 336, "className" => "model.Clients", "methodName" => "saveLog")));
				if(S::$my->error === "") {
					return $log_id;
				}
			}
	...
	public function save($q) {
		...
		if($ref_id = $this->saveLog($q, null)) {
			...
			haxe_Log::trace("" . _hx_string_or_null($cTable) . " fields:" . _hx_string_or_null($cFields->toString()), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 360, "className" => "model.Clients", "methodName" => "save")));
			...
			$sets = new _hx_array(array());
			{
				...
				while($_g < $cFields->length) {
					...
					++$_g;
					$val = $q->get($c);
					haxe_Log::trace(_hx_string_or_null($c) . ":" . _hx_string_or_null(model_Clients::$custom_fields_map->get($c)) . _hx_string_or_null((":" . Std::string($val) . " ")) . Std::string(Util::any2bool(model_Clients::$custom_fields_map->get($c))), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 371, "className" => "model.Clients", "methodName" => "save")));
					if($val !== null) {
			...
			if($sets->length > 0) {
				...
				haxe_Log::trace($sql->b, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 390, "className" => "model.Clients", "methodName" => "save")));
				$success = $stmt->prepare($sql->b);
			} else {
				$success = true;
			}
			if(!$success) {
				haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 396, "className" => "model.Clients", "methodName" => "save")));
				return false;
			}
			...
			if($success) {
				if($customFields2Save) {
					...
					if(!$success) {
						haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 413, "className" => "model.Clients", "methodName" => "save")));
						return false;
					}
				}
				$sql = new StringBuf();
				...
				haxe_Log::trace($sql->b, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 475, "className" => "model.Clients", "methodName" => "save")));
				$success1 = $stmt1->prepare($sql->b);
				if(!$success1) {
					haxe_Log::trace($stmt1->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 479, "className" => "model.Clients", "methodName" => "save")));
					return false;
				}
				$success1 = myBindParam($stmt1, $values2bind, $bindTypes);
				haxe_Log::trace("success:" . Std::string($success1), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 485, "className" => "model.Clients", "methodName" => "save")));
				if($success1) {
					$success1 = $stmt1->execute();
					if(!$success1) {
						haxe_Log::trace($stmt1->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 491, "className" => "model.Clients", "methodName" => "save")));
						return false;
					}
					if(!$this->saveClientData($q)) {
						haxe_Log::trace("oops:" . _hx_string_or_null(S::$my->error), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 496, "className" => "model.Clients", "methodName" => "save")));
					} else {
						return $this->saveLog($q, $ref_id) !== false;
					}
				} else {
					haxe_Log::trace("oops:" . _hx_string_or_null(S::$my->error), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 504, "className" => "model.Clients", "methodName" => "save")));
				}
			}
		}
		return false;
	...
	public function saveClientData($q) {
		...
		$stmt = S::$my->stmt_init();
		haxe_Log::trace($sql->b, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 560, "className" => "model.Clients", "methodName" => "saveClientData")));
		$success = $stmt->prepare($sql->b);
		if(!$success) {
			haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 564, "className" => "model.Clients", "methodName" => "saveClientData")));
			return false;
		}
		haxe_Log::trace($values2bind, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 568, "className" => "model.Clients", "methodName" => "saveClientData")));
		$success = myBindParam($stmt, $values2bind, $bindTypes);
		haxe_Log::trace("success:" . Std::string($success), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 570, "className" => "model.Clients", "methodName" => "saveClientData")));
		if($success) {
			$success = $stmt->execute();
			if(!$success) {
				haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 576, "className" => "model.Clients", "methodName" => "saveClientData")));
				return false;
			}
	...
	public function save_pay_plan($q) {
		...
		$user = S::$user;
		haxe_Log::trace(Std::string($product) . ":" . Std::string(model_Clients_11($this, $product, $q, $user)), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 602, "className" => "model.Clients", "methodName" => "save_pay_plan")));
		$pIt = $product->keys();
		while($pIt->hasNext()) {
			...
			$sql->add("UPDATE fly_crm.pay_plan SET ");
			{
				...
				while($_g < $uFields->length) {
					$c = $uFields[$_g];
					++$_g;
					haxe_Log::trace(_hx_string_or_null($c) . ":" . Std::string(Type::typeof($q->get($c))), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 624, "className" => "model.Clients", "methodName" => "save_pay_plan")));
					$p = $q->get($c);
					$val = null;
					if($p !== null) {
			...
			$stmt = S::$my->stmt_init();
			haxe_Log::trace($sql->b, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 652, "className" => "model.Clients", "methodName" => "save_pay_plan")));
			$success = $stmt->prepare($sql->b);
			if(!$success) {
				haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 656, "className" => "model.Clients", "methodName" => "save_pay_plan")));
				return false;
			}
			haxe_Log::trace($values2bind, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 660, "className" => "model.Clients", "methodName" => "save_pay_plan")));
			$success = myBindParam($stmt, $values2bind, $bindTypes);
			haxe_Log::trace("success:" . Std::string($success), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 662, "className" => "model.Clients", "methodName" => "save_pay_plan")));
			if($success) {
				$success = $stmt->execute();
				if(!$success) {
					haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 668, "className" => "model.Clients", "methodName" => "save_pay_plan")));
					return false;
				}
	...
	public function save_pay_source($q) {
		...
		haxe_Log::trace(Std::string($account) . ":" . Std::string(model_Clients_12($this, $account, $q)), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 695, "className" => "model.Clients", "methodName" => "save_pay_source")));
		$pIt = $account->keys();
		$user = S::$user;
		while($pIt->hasNext()) {
			...
			$sql->add("UPDATE fly_crm.pay_source SET ");
			{
				...
				while($_g < $uFields->length) {
					$c = $uFields[$_g];
					++$_g;
					haxe_Log::trace(_hx_string_or_null($c) . ":" . Std::string(Type::typeof($q->get($c))), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 718, "className" => "model.Clients", "methodName" => "save_pay_source")));
					$p = $q->get($c);
					$val = null;
					if($p !== null) {
						if(!Std::is($p, _hx_qtype("String"))) {
			...
			$stmt = S::$my->stmt_init();
			haxe_Log::trace($sql->b, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 746, "className" => "model.Clients", "methodName" => "save_pay_source")));
			$success = $stmt->prepare($sql->b);
			if(!$success) {
				haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 750, "className" => "model.Clients", "methodName" => "save_pay_source")));
				return false;
			}
			haxe_Log::trace($values2bind, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 754, "className" => "model.Clients", "methodName" => "save_pay_source")));
			$success = myBindParam($stmt, $values2bind, $bindTypes);
			haxe_Log::trace("success:" . Std::string($success), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 756, "className" => "model.Clients", "methodName" => "save_pay_source")));
			if($success) {
				$success = $stmt->execute();
				if(!$success) {
					haxe_Log::trace($stmt->error, _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 762, "className" => "model.Clients", "methodName" => "save_pay_source")));
					return false;
				}
	...
	public function checkOrCreateCustomTable($srcTable, $suffix = null) {
		...
		if(Util::any2bool($res) && $res->num_rows === 0) {
			haxe_Log::trace("CREATE TABLE `" . _hx_string_or_null($newTable) . "` like `" . _hx_string_or_null($srcTable) . "`", _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 780, "className" => "model.Clients", "methodName" => "checkOrCreateCustomTable")));
			...
			haxe_Log::trace("num_rows:" . _hx_string_rec($res->num_rows, ""), _hx_anonymous(array("fileName" => "Clients.hx", "lineNumber" => 791, "className" => "model.Clients", "methodName" => "checkOrCreateCustomTable")));
		}
		return true;
	}
