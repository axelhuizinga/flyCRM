...
class me_cunity_debug_Out {
	...
	static function _trace($v, $i = null) {
		...
		if(me_cunity_debug_Out::$once) {
			...
			me_cunity_debug_Out::_trace("i:" . Std::string(Type::typeof($i)), _hx_anonymous(array("fileName" => "Out.hx", "lineNumber" => 132, "className" => "me.cunity.debug.Out", "methodName" => "_trace")));
			me_cunity_debug_Out::dumpObject($i, _hx_anonymous(array("fileName" => "Out.hx", "lineNumber" => 133, "className" => "me.cunity.debug.Out", "methodName" => "_trace")));
		}
	}
	...
	static function dumpVar($v, $i = null) {
			...
			$ret =  ob_get_clean();
		;
		me_cunity_debug_Out::_trace($ret, _hx_anonymous(array("fileName" => "Out.hx", "lineNumber" => 321, "className" => "me.cunity.debug.Out", "methodName" => "dumpVar")));
	}
		...
		}
		{
			...
			while($_g < $names->length) {
				...
				if(Lambda::has(me_cunity_debug_Out::$skipFields, $name)) {
					$m .= "" . _hx_string_or_null($name) . ":skipped\x0A";
					continue;
				}
				try {
	...
	static function dumpStack($sA, $i = null) {
		...
		haxe_Log::trace($b->b, _hx_anonymous(array("fileName" => "Out.hx", "lineNumber" => 403, "className" => "me.cunity.debug.Out", "methodName" => "dumpStack", "customParams" => (new _hx_array(array($i))))));
	}
...
function me_cunity_debug_Out_0(&$http, &$i, &$msg, &$v, $data) {
	{
		if($data !== "OK") {
			haxe_Log::trace($data, _hx_anonymous(array("fileName" => "Out.hx", "lineNumber" => 201, "className" => "me.cunity.debug.Out", "methodName" => "log2")));
		}
	}
}
