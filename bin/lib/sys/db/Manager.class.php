	...
	public function getUpdateStatement($x) {
		...
		$mod = false;
		{
			...
			while($_g < $_g1->length) {
				...
				if(!_hx_equal($v, $vc) && (!sys_db_Manager_0($this, $_g, $_g1, $cache, $f, $fieldName, $mod, $name, $s, $v, $vc, $x) || sys_db_Manager_1($this, $_g, $_g1, $cache, $f, $fieldName, $mod, $name, $s, $v, $vc, $x))) {
					{
						$_g2 = $f->t;
						switch($_g2->index) {
						case 30:{
							$v = $this->doUpdateCache($x, $name, $v);
							if(!sys_db_Manager_2($this, $_g, $_g1, $_g2, $cache, $f, $fieldName, $mod, $name, $s, $v, $vc, $x)) {
								continue 2;
							}
						}break;
						default:{}break;
...
function sys_db_Manager_0(&$__hx__this, &$_g, &$_g1, &$cache, &$f, &$fieldName, &$mod, &$name, &$s, &$v, &$vc, &$x) {
	switch($f->t->index) {
	case 16:case 22:case 17:case 19:case 18:{
		return true;
	}break;
...
function sys_db_Manager_1(&$__hx__this, &$_g, &$_g1, &$cache, &$f, &$fieldName, &$mod, &$name, &$s, &$v, &$vc, &$x) {
	{
		$a = $v;
		$b = $vc;
...
function sys_db_Manager_2(&$__hx__this, &$_g, &$_g1, &$_g2, &$cache, &$f, &$fieldName, &$mod, &$name, &$s, &$v, &$vc, &$x) {
	{
		$a1 = $v;
		$b1 = $vc;
